let box = document.querySelector(".box"),
  ...
  timerDis = 200, // 定时器间隔
  gameMode = "normal", // 游戏模式 normal/foolish
  snake = {
    width: head.offsetWidth, // 身体格子宽度
    name: "Tommy", // 名字
    head: head, // 蛇头
    parts: [this.head], // 蛇整个身体
    direction: "left", // 蛇头朝向
    lastFoot: {
      x: 0,
      y: 0,
    ...
    move: function () {
      ...
      for (i = this.parts.length - 1; i > 0; i--) {
        this.parts[i].style.left =
          parseInt(getStyle(this.parts[i - 1], "left")) + "px";
        this.parts[i].style.top =
          parseInt(getStyle(this.parts[i - 1], "top")) + "px";
      }
...
function foodRandom() {
  let leftRand =
    parseInt(Math.random() * (box.offsetWidth / snake.width)) * snake.width;
  let topRand =
    parseInt(Math.random() * (box.offsetHeight / snake.width)) * snake.width;
...
function whetherCrashSelf(snake) {
  ...
  for (let i = 1; i < parts.length; i++) {
    if (
      head.style.left == parts[i].style.left &&
      head.style.top == parts[i].style.top
    ) {
      flag = true;
      break;
    }
...
function whetherCrashBoundry(snake) {
  ...
  //碰到右边界
  if (
    parseInt(getStyle(snake.parts[0], "left")) >
      box.offsetWidth - snake.width ||
    //碰到左边界
    parseInt(getStyle(snake.parts[0], "left")) < 0 ||
    //碰到下边界
    parseInt(getStyle(snake.parts[0], "top")) >
      box.offsetHeight - snake.width ||
    //碰到上边界
...
function foolMode(snake) {
  ...
  } else if (parseInt(getStyle(snake.parts[0], "left")) < 0) {
    //碰到左边界
  ...
  } else if (parseInt(getStyle(snake.parts[0], "top")) < 0) {
    //碰到上边界
